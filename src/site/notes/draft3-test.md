---
{"dg-publish":true,"permalink":"/draft3-test/"}
---
...
<div id="flashcards">
  <div class="flashcard" data-answered="none">
    <div class="front">Question 1 long long long long long ? </div>
    <div class="back">Answer 1 long long long long long long long long long long long long long long long long long long long long long long long </div>
    <div class="buttons">
      <button class="correctBtn">Correct</button>
      <button class="incorrectBtn">Incorrect</button>
...
<div id="progress" style="margin-top:15px; font-weight:bold;">0/3 answered</div>
<div id="stats" style="margin-top:20px; display:none; font-weight:bold;"></div>
...
.flashcard:hover { transform: translateY(-5px); box-shadow: 0 8px 20px rgba(0,0,0,0.3); }
.flashcard div.front, .flashcard div.back {
  width: 100%; 
  flex: 1;
  overflow: auto;
...
.correctBtn { background: limegreen; }
.incorrectBtn { background: red; }
.markBtn { background: orange; }
...
function updateProgress() {
  const answered = cards.filter(c => c.dataset.answered !== 'none').length;
  const total = cards.length;
  document.getElementById('progress').innerText = `${answered}/${total} answered`;
}

// Mark answer
function markAnswer(card, answer) {
  card.dataset.answered = answer;
  card.classList.remove('marked');
  updateProgress();
  nextCardOrStats();
...
cards.forEach(card => {
  card.querySelector('.correctBtn').addEventListener('click', () => markAnswer(card,'correct'));
  card.querySelector('.incorrectBtn').addEventListener('click', () => markAnswer(card,'incorrect'));
  card.querySelector('.markBtn').addEventListener('click', () => toggleMark(card));
  card.addEventListener('click', e => {
    if(!e.target.classList.contains('correctBtn') && !e.target.classList.contains('incorrectBtn') && !e.target.classList.contains('markBtn')) {
      toggleCard(card);
    }
  });
});
...
// Next card or show stats
function nextCardOrStats() {
  if(cards.every(c => c.dataset.answered !== 'none')) { showStats(); }
  else { 
    index = (index + 1) % cards.length;
    showCard(index);
  }
...
document.addEventListener('keydown', e => {
  ...
  if(e.key === ' ') { toggleCard(visibleCard); e.preventDefault(); }
  if(e.key.toLowerCase() === 'm') { toggleMark(visibleCard); }
  if(e.key.toLowerCase() === 'c') { markAnswer(visibleCard,'correct'); }
  if(e.key.toLowerCase() === 'i') { markAnswer(visibleCard,'incorrect'); }
  if(e.key === 'ArrowRight') { index = (index+1)%cards.length; showCard(index); }
  if(e.key === 'ArrowLeft') { index = (index-1+cards.length)%cards.length; showCard(index); }
});
